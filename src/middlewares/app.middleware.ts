...
export class AppMiddlewares {
	...
	public static async SecureApiRoutesWithValidateSignature(
		req: Request,
		res: Response,
		next: NextFunction,
	) {
		try {
			if (
				!req.headers[sigHeaderName.toLowerCase()] ||
				req.headers[sigHeaderName.toLowerCase()] === null
			) {
				res
					.status(400)
					.json({
			...
			const digest = await new Security().GenerateSignature(
				method,
				originalUrl,
			...
			res
				.json({ message: "Something went wrong", result: null, success: false })
				.end();
			return;
		}
	}
