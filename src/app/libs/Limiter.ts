...
export class Limiter {
    ...
    static useTokenLimiter() {
        ...
        return (req: Request, res: Response, next: NextFunction) => rateLimit.consume(req.ip!, 1).then(() => next()).catch((rateLimiterRes) => {
    ...
    tokenBucketMiddleware() {
        return (req: Request, res: Response, next: any) => {
            if (tokenBucket.length > 0) {
                ...
                return res.status(429).set('X-RateLimit-Remaining', "0").set('Retry-After', "2").json({
                    success: false,
                    name: "TOO_MANY_REQUESTS",
                    message: "Current Rate Limit is Exceeded",
                    stack: { path: req.path, info: "Request Throttled", solution: "Try Again after time you settled in the rate limit, or disable it" }
                });
            }
        }
    }
