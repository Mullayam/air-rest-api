...
export class FilesMapper {
	private static extractExports(importedModule: any): any[] {
		return Object.values(importedModule).filter(
	...
	private static async getFilesRecursively(
		...
		const files = entries.map((entry) => {
			...
			return entry.isDirectory()
				? FilesMapper.getFilesRecursively(res)
				: Promise.resolve(res);
		});
	...
	static async _importModules(path: string): Promise<any[]> {
		const files = path.includes("*")
			? globSync(path)
			: await FilesMapper.getFilesRecursively(path);
		const modules: any[] = [];
		for (const file of files) {
			if (
				...
				try {
					...
					modules.push(...FilesMapper.extractExports(importedModule));
				} catch (error) {
		...
		const result = fs
			...
			.filter(
				(file) =>
					...
					(file !== ".ts" && !file.endsWith(".d.ts")),
			)
			.filter(
				(file) => file.indexOf(".spec") === -1 && file.indexOf(".test") === -1,
			);

		if (type === "default") {
			modules = result.map(
				(file) => require(`${__workingDir}/${file}`).default as any,
			);
			return modules;
		}
