...
export class MailService {
	...
	async SendMail({
		...
		return await this.transporter
			...
			.then((info) => {
				Logging.info(
					` Mail sent successfully to ${to} [MailResponse]=${info.response} [MessageID]=${info.messageId}!!`,
				);
				return info;
			});
	}
	...
	async SendTemplate({
		...
		context,
		from,
	}: Omit<MailOptionsWithTemplate, "text" | "html">): Promise<SentMessageInfo> {
		if (!from) {
		...
		return await this.transporter.sendMail(PrepareEmailOptions).then((info) => {
			Logging.info(
				` Mail sent successfully to ${to} [MailResponse]=${info.response} [MessageID]=${info.messageId}!!`,
			);
			return info;
